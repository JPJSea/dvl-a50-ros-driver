...
def getData():
    ...
    while not "\n" in raw_data:
        try:
            ...
            rospy.logerr(
                "Lost connection with the DVL, reinitiating the connection: {}".format(
                    err
                )
            )
            connect()
        ...
        raw_data = (
            raw_data + rec.decode()
        )  # This is a fix for python3 TODO check still works with melodic
...
def publisher():
    ...
    while not rospy.is_shutdown():
        ...
        theDVL.fom = data["fom"]
        theDVL.covariance = [val for row in data["covariance"] for val in row]
        theDVL.altitude = data["altitude"]
        theDVL.velocity_valid = data["velocity_valid"]
        theDVL.status = data["status"]
        theDVL.form = data["format"]
        theDVL.time_of_validity = data["time_of_validity"]
        theDVL.time_of_transmission = data["time_of_transmission"]
